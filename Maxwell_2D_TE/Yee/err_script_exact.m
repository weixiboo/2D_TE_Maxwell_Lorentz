...
%Double check yee_2d_TE.m if souce pulse is off
%Double check yee_2d_TE.m if initial condition is the exact solution
K = 1;
T = 0.25;
...
for i = 1:length(N)
    H_exact = sin(sqrt(2)*pi*T).*cos(pi*X{i}).*cos(pi*Y{i});
