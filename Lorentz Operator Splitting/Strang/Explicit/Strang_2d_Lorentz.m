...
function  [delx,dely,delt,X_dual,Y_dual,H_z_new] = Strang_2d_Lorentz(k_mac)
...
delt= (0.99/c)*(1/sqrt((1/delx^2)+(1/dely^2))); 
...
for i = 1:NT
    ...
    %Step A:
    H_z_new = H_z_old - (delt/4)./mu0.*(diff(E_y_old,1,2)./delx - diff(E_x_old,1,1)./dely);
        
    % H_z_new(src_loc:src_loc+1,src_loc:src_loc+1) = ...
    %     H_z_new(src_loc:src_loc+1,src_loc:src_loc+1)...
    %     + (delt)./(delx*dely)*(1e-2)*b((i-7/8)*delt,f_bw);

    E_x_new = E_x_old + ((delt/2)./(dely*eps0.*eps_inf)).*matrix_padding(diff(H_z_new,1,1),1);
    E_y_new = E_y_old - ((delt/2)./(delx*eps0.*eps_inf)).*matrix_padding(diff(H_z_new,1,2),2);

    H_z_new = H_z_new - (delt/4)./mu0.*(diff(E_y_new,1,2)./delx - diff(E_x_new,1,1)./dely);
    
    % H_z_new(src_loc:src_loc+1,src_loc:src_loc+1) = ...
    %     H_z_new(src_loc:src_loc+1,src_loc:src_loc+1)...
    %     + (delt)./(delx*dely)*(1e-2)*b((i-5/8)*delt,f_bw);
    ...
    % H_z_new(src_loc:src_loc+1,src_loc:src_loc+1) = ...
    %     H_z_new(src_loc:src_loc+1,src_loc:src_loc+1)...
    %     + (delt)./(delx*dely)*(1e-2)*b((i-3/8)*delt,f_bw);

    E_x_new = E_x_new + ((delt/2)./(dely*eps0.*eps_inf)).*matrix_padding(diff(H_z_new,1,1),1);
    E_y_new = E_y_new - ((delt/2)./(delx*eps0.*eps_inf)).*matrix_padding(diff(H_z_new,1,2),2);

    H_z_new = H_z_new - (delt/4)./mu0.*(diff(E_y_new,1,2)./delx - diff(E_x_new,1,1)./dely);
    
    % H_z_new(src_loc:src_loc+1,src_loc:src_loc+1) = ...
    %     H_z_new(src_loc:src_loc+1,src_loc:src_loc+1)...
    %     + (delt)./(delx*dely)*(1e-2)*b((i-1/8)*delt,f_bw);
