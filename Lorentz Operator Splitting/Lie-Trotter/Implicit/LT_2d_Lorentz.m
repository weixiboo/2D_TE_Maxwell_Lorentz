...
Maxwell_x = maxwell_step_matrix(eps0*eps_inf,mu0,dely,delt,size(E_x_old,1));
Maxwell_y = maxwell_step_matrix(eps0*eps_inf,mu0,delx,delt,size(E_y_old,2));
...
for i = 1:NT
    ...
    %Step A1: 
    b_Ex = E_x_old + (mu0*(c_inf.^2)*delt/dely).*matrix_padding(diff(H_z_old,1,1),1) ...
        + (((c_inf)*delt/dely).^2)/4.*matrix_padding(diff(E_x_old,2,1),1);
    E_x_new = Maxwell_x\b_Ex;
    H_z_new = H_z_old + delt./mu0./dely./2.*(diff(E_x_new,1,1) + diff(E_x_old,1,1));

    %Step A2:
    b_Ey = E_y_old - (mu0*(c_inf.^2)*delt/delx).*matrix_padding(diff(H_z_old,1,2),2) ...
        + (((c_inf)*delt/delx).^2)/4.*matrix_padding(diff(E_y_old,2,2),2);
    b_Ey = transpose(b_Ey);
    E_y_new = transpose(Maxwell_y\b_Ey);
    H_z_new = H_z_new - delt./mu0./delx./2.*(diff(E_y_new,1,2) + diff(E_y_old,1,2));
